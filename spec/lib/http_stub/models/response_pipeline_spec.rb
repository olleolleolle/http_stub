describe HttpStub::Models::ResponsePipeline do

  let(:server)            { instance_double(HttpStub::Server) }

  let(:response_pipeline) { HttpStub::Models::ResponsePipeline.new(server) }

  describe "#process" do

    let(:response) { double(HttpStub::Models::StubResponse::Base, delay_in_seconds: 5, serve_on: nil) }
