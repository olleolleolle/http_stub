...
When matching a request, a stubs uri, header values, parameter values or body can be regular expressions:
...
```ruby
  ...
  stub_server.add_stub! do
    match_requests(/prefix\/[^\/]+\/postfix/, method: :post, body: /^some body$/)
    with_response(status: 201)
  end
```

Header and parameter values can also be mandatory omissions, for example:
...
Request bodies can be exactly matched, for example:

```ruby
  stub_server.add_stub! do
    match_requests("/some/path", method: :post,
                                 body:   "some precise content")
    with_response(status: 201)
  end
```

Request bodies can be matched as conforming to a JSON schema, for example:

```ruby
  stub_server.add_stub! do
    match_requests("/some/path", method: :post,
                                 body:   schema(:json, type:       :object,
                                                       properties: { some_attribute: { type: :string })
    with_response(status: 201)
  end
```

Given schema definitions are verbose, we recommend externalizing your schema definition to a dedicated file:

```ruby
  stub_server.add_stub! do
    match_requests("/some/path", method: :post,
                                 body:   schema(:json, JSON.parse(File.read("/path/to/schema/file.json")))
    with_response(status: 201)
  end
```
...
Server tasks generated by ```http_stub``` will initialize a configurer when provided:
